...
class Camera:
    ...
    @use_threading.setter
    def use_threading(self, value: bool):
        self._use_threading = bool(value)

    def _writer_worker(self, queue, tif_writer, metadata_list):
        """Background thread that writes frames to disk"""
        frame_count = 0
        while not self._stop_writer.is_set() or not queue.empty():
            try:
                item = queue.get(timeout=0.1)
                if item is None:  # Sentinel value
                    break

                image_data, timestamp = item
                ...
                metadata_list.append(timestamp)
                frame_count += 1

                if self.debug:
                    self.log(f'Written frame {frame_count}')

                queue.task_done()

            except Exception as e:
                if not self._stop_writer.is_set():
                    self.log(f'Writer thread error: {e}', logging.ERROR)

        self.log(f'Writer thread finished. Wrote {frame_count} frames.')
    ...
    def set_buffers(self, num_buffers='max'):
        ...
        if num_buffers == 'max':
            num_buffers_to_set = buffer_count.GetMax()
        else:
    ...
    def acquire_images(self, use_processor=False):
        try:
            # MAX 1440
            # self.set_buffers(1576)
            self.set_buffers(num_buffers='max')

            # Set pixel format to Mono8 natively
            if self._cam.PixelFormat.GetAccessMode() == PySpin.RW:
                self._cam.PixelFormat.SetValue(PySpin.PixelFormat_Mono8)
                self.log('Set pixel format to Mono8')
            ...
            if self.acquisition_mode == 'single frame':
                ...
                try:
                    ...
                    if image_result.IsIncomplete():
                        ...
                        filename = '%s-%d.jpg' % (image_prefix, 1)
    ...
    def _acquire_images_direct(self, tif_file_name, timeout, image_prefix):
                ...
                try:
                    ...
                    if image_result.IsIncomplete():
                        self.log('Image incomplete with image status %d...' %
                                 image_result.GetImageStatus(), logging.WARNING)
                    else:
                        chunk_data = image_result.GetChunkData()
                        timestamp = chunk_data.GetTimestamp()

                        # Get numpy array directly (already Mono8 from camera)
                        image_array = image_result.GetNDArray().copy()
                    ...
                    self.log(f'Error acquiring image {i + 1}/{self._number_of_images}: {ex}',
                             logging.ERROR)

    def _acquire_images_threaded(self, tif_file_name, timeout, image_prefix):
        ...
        # Open TIFF writer
        tif_writer = tifffile.TiffWriter(str(tif_file_name), imagej=False)

        # Start writer thread
        self._writer_thread = Thread(
            target=self._writer_worker,
            args=(self._frame_queue, tif_writer, self._timestamps),
            daemon=False
        )
        self._writer_thread.start()
        self.log('Started writer thread')
        ...
        for i in range(self._number_of_images):
            ...
            try:
                ...
                if image_result.IsIncomplete():
                    ...
                    # Get numpy array and COPY it (critical!)
                    image_array = image_result.GetNDArray().copy()

                    # Queue for writing
                    self._frame_queue.put((image_array, timestamp))

                    if self.debug and (i + 1) % 50 == 0:
                        self.log(f'Queued image {i + 1:>03d}/{self._number_of_images:>03d}')
        ...
        self.log(f'Acquisition complete. Dropped frames: {dropped_frames}')
        self.log('Waiting for writer thread to finish...')
        ...
        if dropped_frames > 0:
            self.log(f'WARNING: Dropped {dropped_frames} frames during acquisition',
                     logging.WARNING)
    ...
    def device_serial_number(self) -> str:
        try:
            ...
            if self._cam.TLDevice.DeviceSerialNumber is not None and self._cam.TLDevice.DeviceSerialNumber.GetAccessMode() == PySpin.RO:
                device_serial_number = self._cam.TLDevice.DeviceSerialNumber.GetValue()
        except PySpin.SpinnakerException as ex:
            self.log(f'Error: {ex}', logging.ERROR)
            return ''
