...
if __name__ == '__main__':
    ...
    min_recession = df['Recession rate (cm/s)'].min()
    max_recession = df['Recession rate (cm/s)'].max()
    ...
    for i, m in enumerate(labels):
        ...
        print(f"Label: {m}, {filler_wtp:>5.2f} wt % filler")
        lbl = f"{filler_wtp:.2f} wt % Filler"
        ...
        ax.errorbar(
            ...
            capsize=2.75, elinewidth=1.25, lw=1.5, c=c
        )
    ...
    fig.savefig(os.path.join(base_dir, 'recession_rate_vs_laser_power_filler_content' + '.svg'), dpi=600)
    fig.savefig(os.path.join(base_dir, 'recession_rate_vs_laser_power_filler_content' + '.png'), dpi=600)
    fig.savefig(os.path.join(base_dir, 'recession_rate_vs_laser_power_filler_content' + '.pdf'), dpi=600)
