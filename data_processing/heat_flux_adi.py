...
def simulate_adi_temp(laser_power: float, r_holder: float, r_sample: float,
    ...
    t_max = float(kwargs.get('t_max', 2.0))
    pulse_length = float(kwargs.get('pulse_length', 1.0))
    chi = float(kwargs.get('chi', 1.0))
    emissivity = float(kwargs.get('emissivity', 1.0))
    beam_diameter = float(kwargs.get('beam_diameter', 0.8165))
    holder_thickness = float(kwargs.get('holder_thickness_cm', 2.54))
    T_a = float(kwargs.get('T_a', 20.0))
    report_every = int(kwargs.get('report_every', 10))
    debug = bool(kwargs.get('debug', False))
    save_h5 = bool(kwargs.get('save_h5', False))
    x_tc_1 = float(kwargs.get('x_tc_1', 1.0))
    x_tc_2 = float(kwargs.get('x_tc_2', 2.0))
    R = r_holder
    R_sample = r_sample
    ...
    # q = 1.0E3*gaussian(r, 0, 0.15)
    q = gaussian_beam(r=r, beam_power=laser_power, beam_diameter=beam_diameter)
    q[r > R_sample] = 0.0

    hf_filename = f'./ADI_k1{kappa_1:.2E}_k2_{kappa_2:.2E}_P{laser_power:.2E}'
    ...
    A_1 = sp.sparse.diags(diagonals, [-1, 0, 1], format='csc')
    ...
    diagonals = [d_0, d_1, d_2]
    A_2 = sp.sparse.diags(diagonals, [-1, 0, 1], format='csc')
    ...
    solve_A1 = factorized(A_1)
    solve_A2 = factorized(A_2)
    ...
    eye_n = np.ones(N + 1, dtype=np.float64)
    d = np.zeros(M + 1, dtype=np.float64)
    b = np.zeros(N + 1, dtype=np.float64)
    msk_exposed = x < holder_thickness
    ...
    def tfourth(coefficient: float, x: np.ndarray) -> np.ndarray:
        return (coefficient ** 4.0) * (np.power(c2k(x), 4.0) - T_a_4)

    def c2k(x: np.ndarray) -> np.ndarray:
        return x + 273.15
    ...
    while t_now <= t_max:
        if count % (report_every - 1) == 0 and debug:
            print(
                f'(1) Time: {t_now:4.3f} s, T(r={r[idx_r - 1]:3.3f}, x={x[idx_x1]:3.3f}) = {U_k1[idx_r - 1, idx_x1]:5.1f}, T(r={r[idx_r - 1]:3.3f}, x={x[idx_x2]:3.3f}) = {U_k1[idx_r - 1, idx_x2]:5.1f}, T(r={r[-1]:3.3f}, x={x[idx_h]:3.3f}) = {U_k1[-1, idx_h]:5.1f}, Wall Time: {(time.time() - start_time) / 60.0:8.2f} min, Step Time: {(time.time() - step_time):6.3E} s')

        temperature_p1[count] = U_k1[idx_r, idx_x1]
        ...
        for j in range(0, N + 1):
            d[-1] = tfourth(z_cal_1_4, U_k1[M, j]) if j < idx_h else tfourth(z_cal_2_4, U_k1[M, j])

            if j == 0:
                if t_now <= pulse_length:
                    w1 = (eye_m - 2.0 * mu_j) * U_k1[:, 0] + 2.0 * mu_j * (
                                U_k1[:, 1] + dx * w_j * chi * q - dx * w_j_0 * tfourth(sb4, U_k1[:, j])) + d
                else:
                    w1 = (eye_m - 2.0 * mu_j) * U_k1[:, 0] + 2.0 * mu_j * (
                                U_k1[:, 1] - dx * w_j_0 * tfourth(sb4, U_k1[:, j])) + d
            elif j == N:
                w1 = 2.0 * mu_j * (U_k1[:, j - 1] + dx * w_j * tfourth(sb4, U_k1[:, j])) + (eye_m - 2.0 * mu_j) * U_k1[
                                                                                                                  :,
                                                                                                                  j] + d
            else:
                ...
                if j < idx_h:
                    ...
                    if t_now <= pulse_length:
                        w_e = (eye_m - 2.0 * mu_j) * U_k1[:, idx_h] + 2.0 * mu_j * (
                                    U_k1[:, idx_h + 1] + dx * w_j * chi * q - dx * w_j * tfourth(sb4,
                                                                                                 U_k1[:, idx_h])) + d
                    else:
                        w_e = (eye_m - 2.0 * mu_j) * U_k1[:, idx_h] + 2.0 * mu_j * (
                                    U_k1[:, idx_h + 1] - dx * w_j * tfourth(sb4, U_k1[:, idx_h])) + d
                    w1[idx_r + 2:] = w_e[idx_r + 2:]
            ...
            if (np.isnan(w1).any() or np.isinf(w1).any()) and debug:
                ...
                for k in range(M + 1):
                    if idx_nan[k]:
                        print(f'r = {r[k]:.3f} cm, w1 = {w1[k]}, U_kh = {U_k1[j, k]}')
        ...
        for i in range(0, M + 1):
            g_i = 2.0 * dx * (chi * w_j[i] * q[i] - w_j[i] * tfourth(sb4, U_kh[
                i, 0])) if t_now <= pulse_length else -2.0 * dx * w_j[i] * tfourth(sb4, U_kh[i, 0])
            b_n = s_cal_1 * tfourth(sb4, U_kh[i, 0]) if i <= idx_r else s_cal_2 * tfourth(sb4, U_kh[i, 0])
            b[0] = g_i
            b[-1] = b_n
            ...
            if i == 0:
                ...
                w2 = (beta_1 * (1.0 + k_cal_4) - (gamma_1 / r[idx_r]) * (1.0 - k_cal_4)) * U_kh[idx_r - 1, :] + (
                ...
                w2[:idx_h] = 2.0 * beta_1 * U_kh[i - 1, :idx_h] + (1.0 - 2.0 * beta_1) * U_kh[i, :idx_h] + (
                            beta_1 + gamma_1 / R_sample) * (2.0 * dr / k0_1) * tfourth(sb4, U_kh[i, :idx_h]) + mu_1 * b[
                                                                                                                      :idx_h]
            elif i == (idx_r + 1):
                w2 = (beta_2 * (1.0 + k_cal_4) - (gamma_2 / r[idx_r + 1]) * (1.0 - k_cal_4)) * U_kh[idx_r, :] + (
                            1.0 - beta_2 * (2.0 - k_cal_3) + k_cal_3 * (gamma_2 / r[idx_r + 1])) * U_kh[idx_r + 1,
                                                                                                   :] + mu_2 * b
                w2[:idx_h] = U_kh[i, :idx_h]
            elif i == M:
                w2 = 2.0 * beta_2 * U_kh[i - 1, :] + (1.0 - 2.0 * beta_2) * U_kh[i, :] + (beta_2 + gamma_2 / R) * (
                            2.0 * dr / k0_2) * tfourth(sb4, U_kh[M, :]) + mu_2 * b
                w2[:idx_h] = U_kh[i, :idx_h]
