...
def main():
    ...
    for k, p in enumerate(pids):
        ...
        if p in plot_pids:
            lbl = f'{round(heat_loads_mapping[laser_setting] / 5.) * 5.} MW/m$^{{\mathregular{{2}}}}$'
            ax.plot(t - t0, temperature, lw=1.5, color=laser_setting_color_map[laser_setting], alpha=0.75)
            ax.plot(
                [t_takeoff - t0], [temp_takeoff], marker='o', markersize=4, mew=0.75, fillstyle='full',
                color=laser_setting_color_map[laser_setting], mfc='k',
                mec=laser_setting_color_map[laser_setting], alpha=0.5, label=lbl
            )
    ...
    fig.savefig(os.path.join(base_path, save_dir, 'pebble_temperature.svg'), dpi=600)
    ...
    selected_cooling_df.to_csv(os.path.join(base_path, save_dir, 'selected_cooling_data.csv'), index=False, encoding='utf-8-sig')
