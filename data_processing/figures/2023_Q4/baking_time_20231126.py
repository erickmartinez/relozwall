...
def main():
    ...
    agg_df = merged_df.groupby(by=['Baking time (min)']).agg({
            ...
            'Graphite sublimation (Torr-L/s/m^2)': ['mean', 'std', 'count'],
        })
    ...
    yerr_g = 0.5 * agg_df['Graphite sublimation (Torr-L/s/m^2)']['mean'].values
    ...
    axes[0].errorbar(
        ...
        yerr=yerr_s,
        c='tab:red',
        marker='o', lw=1.5, ls='none',
        # mec='k',
        mew=1.25, capsize=3.5, capthick=1.25, ecolor='tab:red', fillstyle='none',
        label='Matrix'
    )

    axes[0].errorbar(
        agg_df['Baking time (min)']['first'],
        agg_df['Graphite sublimation (Torr-L/s/m^2)']['mean'],
        yerr=yerr_g,
        c='tab:green',
        marker='s', lw=1.5, ls='none',
        # mec='k',
        mew=1.25, capsize=3.5, capthick=1.25, ecolor='tab:green', fillstyle='left',
        label='Graphite'
    )
    ...
    axes[0].errorbar(
        ...
        c='tab:blue',
        marker='^', lw=1.5, ls='none',
        # mec='k',
        mew=1.25, capsize=3.5, capthick=1.25, ecolor='tab:blue', fillstyle='full',
        label='(CH$_{\\mathregular{x}})$'
    )
