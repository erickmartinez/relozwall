...
cp_csv = 'Medwick AIP1991 - Specific heat of amorphous boron.csv'
...
def main():
    ...
    cpbt3 = df['C/T^3 (J/g-K^4)'].values

    # fit the values to poly
    b0 = np.array([-1., -1E-3, -1., -1.])
    all_tol = np.finfo(np.float64).eps
    n = len(cpbt3)
    log_t, log_c = np.log10(temperature_k), np.log10(cpbt3)
    res = least_squares(
        fobj,
        b0,
    ...
    ypred, lpb, upb = cf.predint(x=xpred, xd=temperature_k, yd=cpbt3, func=poly, res=res)
    ...
    ax.plot(temperature_k, cpbt3, color='C0', ls='none',  marker='o', mfc='none', label='Experiment')
    ax.plot(10**xpred, 10**ypred, color=lighten_color('C0', 1.25), label='Model')

    t300 = 300.
    cp_300 = 10**poly(np.log10(np.array([t300])), res.x)
    cp_300 = cp_300[0]

    ax.set_xlabel('Temperature [K]')
    ax.set_ylabel('$\mathregular{C/T^3}$ (J g$^{\mathregular{-1}}$ K$^{\mathregular{-4}}$)')
    ...
    print(f'CP(T=300K)/T^3, {cp_300:.2E} (J/g/K^4)')
    print(f'CP(T=300K), {cp_300*t300**3.:.2E} (J/g-K)')

    fig.savefig(os.path.join(base_dir, 'Medwick1991_a-boron_cp.png'), dpi=300)
