...
def main():
    ...
    tracking_summary_df = pd.DataFrame(data={
        ...
        'Graphite sublimation rate (Torr-L/s/m^2)': [x for x in tracking_df_agg['Graphite sublimation rate (Torr-L/s/m^2)']['mean']],
    ...
    for i, lps in enumerate(laser_power_settings):
        ...
        print(g_df[['ROW', 'Big spheres','Graphite sublimation rate (Torr-L/s/m^2)']])
        axes[0].errorbar(
            g_df['Big spheres'], g_df['Graphite sublimation rate (Torr-L/s/m^2)'],
            yerr=g_df['Graphite sublimation rate error (Torr-L/s/m^2)'],
            c=colors_dr[i],
            marker=markers_dr[i], lw=1.5, ls='none',
            # mec='k',
        ...
        lbl_h = f'{5 * round(lp / 5):.0f} MW/m$^{{\\mathregular{{2}}}}$ (CH$_{{\\mathregular{{3}}}}$)'
