...
out_dir = r'C:\Users\erick\OneDrive\Documents\ucsd\ARPA-E\DIII-D'
...
if __name__ == '__main__':
    ...
    for i, m in enumerate(labels):
        print(f'Label: {m}')
        df2 = df[df['Label'] == m]
        tc = df2['Thermal conductivity (W/cm-K)'].mean()
        ...
        ax.errorbar(
            ...
            marker=markers[i], ms=9, mew=1.25, mfc='none', label=f'${latex_float(tc)}$ (W/cm-K)',
    ...
    fig.savefig(os.path.join(out_dir, 'recession_rate_vs_laser_power' + '.svg'), dpi=600)
    fig.savefig(os.path.join(out_dir, 'recession_rate_vs_laser_power' + '.png'), dpi=600)
    fig.savefig(os.path.join(out_dir, 'recession_rate_vs_laser_power' + '.pdf'), dpi=600)
