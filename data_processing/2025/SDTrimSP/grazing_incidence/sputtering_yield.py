...
def estimate_w(e_l):
    ...
    float
    """
    # return e_l + 0.1728 * np.sqrt(e_l) + 0.008*(e_l**0.1505) # Ecksteing paper uses 0.1504!
    return e_l + 0.1728 * np.sqrt(e_l) + 0.008*(e_l**0.1504)
...
def estimate_yield_eckstein_preuss(E0, q, lam, E_th, mu, m1, m2, z1, z2, sbe):
    ...
    E0Eth = np.ones_like(E0)*1E-8
    E0Eth[msk_above_eth] = (E0[msk_above_eth]/E_th - 1.0) ** mu
    # y = q * snkr * E0Eth / (lam / w + E0Eth) # < Eckstein W, Vacuum 2008;82;930
    y = q * snkr * E0Eth / (lam + E0Eth) # < Eckstein W, Preuss R. J Nucl Mater 2003;320:209.
    return y

def angle_of_incidence_factor(E0, angle, f, b, c, E_sp=1.):
    """
    Estimates the factor for the angular dependence of the sputtering yield
    according to the Eckstein-Preuss model
    ...
    ----------
    E0: np.ndarray
        The energy of the projectiles (eV)
    angle: float
        The angle of incidence measured with respect to the normal
    f: float
        Fit parameter
    b: float
        Fit parameter
    c: float
        Fit parameter
    E_sp: float
        Surface binding energy of the projectiles:
        E_sp = 0 for noble gas projectiles
        E_sp = 1 eV for hydrogen isotopes is assumed

    Returns
    -------
    np.ndarray:
        The angular dependence factor
    """
    alpha_0 = np.pi - np.arccos(np.sqrt(1. / (1. + E0/E_sp)))
    argument = (angle/alpha_0 * np.pi / 2.) ** c
    factor_1 = (np.cos(argument)) ** (-f)
    factor_2 = np.exp(b * (1. - 1. / np.cos(argument)))
    return factor_1 * factor_2

def estimate_yield_eckstein_preuss_with_angle_dependence(
    E0, q, lam, E_th, mu, m1, m2, z1, z2, sbe, angle, f, b, c, E_sp=1.
):
    normal_incidence_yield = estimate_yield_eckstein_preuss(
        E0, q, lam, E_th, mu, m1, m2, z1, z2, sbe,
    )

    angular_dependence = angle_of_incidence_factor(
        E0, angle, f, b, c, E_sp
    )

    return normal_incidence_yield * angular_dependence
