...
PATH_TO_BI_SPUTTER_EH = r'./data/ehollmann/BI_sput_file.txt'
...
def plot_bi_sput(ax:plt.Axes, t_hits, path_to_bi_sput=PATH_TO_BI_SPUTTER_EH, t_range=T_RANGE, color='C5'):
    shot = 203782
    # Load the data for OSP B source estimated from core B-V CER data #203780 and EDGE2D Pinj=1.5 MW case
    bi_sputr_df = pd.read_csv(path_to_bi_sput, sep=r'\s+', skiprows=3, names=['time [s]', 'Bdot(Ysput)[1e18 B/s]']).apply(
        pd.to_numeric, errors='coerce')
    t_sput = bi_sputr_df['time [s]'].values
    b_sput = bi_sputr_df['Bdot(Ysput)[1e18 B/s]'].values

    msk_time = (t_range[0] <= t_sput) & (t_sput <= t_range[1])
    t_cer = t_sput[msk_time]
    b_dot = b_sput[msk_time]
    spl_bsput = make_smoothing_spline(x=t_sput, y=b_sput, lam=0.0002)

    idx_peaks, _ = find_peaks(spl_bsput(t_sput), prominence=1, distance=20)
    idx_peaks = np.array([idx_peak for idx_peak in idx_peaks if
                          (t_range[0] <= t_sput[idx_peak]) & (t_sput[idx_peak] <= t_range[1])])

    for idx_peak in idx_peaks:
        print(f'Sputtering peak: {t_sput[idx_peak]:.3f} s')
    for t_hit in t_hits:
        print(f'203782 OSP hit: {t_hit:.3f} s')
    # dt = t_cer[idx_peaks[0]] - t_hits[0]
    t_plot = np.linspace(t_range[0], t_range[1], 500)

    handle, = ax.plot(t_plot, spl_bsput(t_plot), color=color, label='BI (Ysput)')
    return handle
...
def main(
    ...
    fig, axes = plt.subplots(nrows=4, ncols=1, sharex=True, height_ratios=[1, 1, 1, 1])
    ...
    line_bd = plot_mds_mass_loss(shot=shot_bd_rate, ax=axes[3], color='C5', label='Local BD', scale=1E5, lam=1E-6)

    plot_evaporation(shot=shot_evaporation, ax=axes[3], label=f'Evaporation', color='C2', scale=1E5)
    ...
    plot_bi_sput(ax=axes[2], t_hits=t_hits, color='C6')
    ...
    axes[3].yaxis.set_minor_locator(ticker.MultipleLocator(2.5))
    ...
    axes[-2].set_ylabel(r'$\Gamma_{\mathrm{B}}$ ($\times 10^{18}$ atoms/s)', usetex=True)

    fig.align_labels()
    path_to_figures = Path(r'./figures')

    fig.savefig(path_to_figures / 'boron_emission_rates_eh.svg', dpi=600, bbox_inches='tight')
