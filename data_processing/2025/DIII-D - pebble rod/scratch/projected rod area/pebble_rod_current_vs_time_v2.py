...
def smooth_voltage_and_convert_current(csv_file, resistance=0.31, smoothing_method='savgol', **kwargs):
    # 5. RC filter (simulates hardware RC low-pass filter)
    # df_rc = smooth_voltage_and_convert_current(
    #     csv_file,
    #     resistance=resistance,
    #     smoothing_method='rc',
    #     tau=0.05  # Time constant in seconds - smaller = more smoothing
    # )

    # 6. Exponential smoothing
    # df_exp = smooth_voltage_and_convert_current(
    #     csv_file,
    #     resistance=resistance,
    #     smoothing_method='exponential',
    #     alpha=0.1  # Smoothing factor (0-1) - smaller = more smoothing
    # )
    """"
    ...
    **kwargs : additional parameters for smoothing methods
    ...
    if smoothing_method == 'savgol':
    ...
    elif smoothing_method == 'rc':
        # RC filter smoothing (simulates hardware RC low-pass filter)
        tau = kwargs.get('tau', 0.1)  # Time constant in seconds
        dt = np.mean(np.diff(df['t_s']))  # Average time step
        alpha = dt / (tau + dt)  # Smoothing factor

        voltage_smooth = np.zeros_like(voltage_raw)
        voltage_smooth[0] = voltage_raw[0]

        for i in range(1, len(voltage_raw)):
            voltage_smooth[i] = alpha * voltage_raw[i] + (1 - alpha) * voltage_smooth[i-1]

        print(f"Applied RC filter (tau={tau}s, alpha={alpha:.4f})")

    elif smoothing_method == 'exponential':
        # Exponential smoothing (similar to RC but with different parameterization)
        alpha = kwargs.get('alpha', 0.1)  # Smoothing factor (0-1)
        voltage_smooth = np.zeros_like(voltage_raw)
        voltage_smooth[0] = voltage_raw[0]
        ...
        print(f"Applied exponential smoothing (alpha={alpha})")

    else:
        raise ValueError("smoothing_method must be 'savgol', 'gaussian', 'moving_average', 'butter', 'rc', or 'exponential'")
...
if __name__ == "__main__":
    ...
    df_savgol = smooth_voltage_and_convert_current(
        ...
        resistance=resistance,
        smoothing_method='rc',
        window_length=501,  # Adjust for more/less smoothing
        polyorder=3,
        tau=10
    )
