...
if __name__ == "__main__":
    test_cases = [
        (1.234, 0.056),  # Regular floating point
        (0.00123, 0.00045),  # Small numbers, scientific notation
        (1234.5, 67.8),  # Large numbers, scientific notation
        (0.0456, 0.0023),  # Small but within default limits
        (99.7, 2.3),  # Large but within default limits
        (1.0e-5, 2.3e-6),  # Very small numbers
        (5.67e6, 1.2e5),  # Very large numbers
        (0, 0.1),  # Zero value
        (-0.00234, 0.00012),  # Negative value
    ]

    print("Default power limits (2, 2):")
    for value, uncertainty in test_cases:
        result = format_value_uncertainty_latex(value, uncertainty)
        print(f"Value: {value:12.6g}, Uncertainty: {uncertainty:12.6g} -> {result}")

    print("\nCustom power limits (3, 3):")
    for value, uncertainty in test_cases[:5]:
        result = format_value_uncertainty_latex(value, uncertainty, power_limits=(3, 3))
        print(f"Value: {value:12.6g}, Uncertainty: {uncertainty:12.6g} -> {result}")