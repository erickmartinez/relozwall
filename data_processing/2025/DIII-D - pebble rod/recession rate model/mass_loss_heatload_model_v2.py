...
def main(
    ...
    load_plot_style()
    fig, axes = plt.subplots(nrows=3, ncols=2, constrained_layout=True, sharex=False)
    ...
    fig_h, axes_h = plt.subplots(nrows=2, ncols=2, constrained_layout=True, sharex=True)
    fig_h.set_size_inches(7.5, 5)
    ...
    for i, shot in enumerate(shots):
        ...
        h = np.ones_like(dm_dt) * h0
        for j in range(len(dm_dt)):
            if j < 5:
                h[j] = h0_i - 4 * trapezoid(y=dm_dt[0:j], x=t_shot[0:j])/(rho * diameter ** 2)
            else:
                h[j] = h0_i - 4 * simpson(y=dm_dt[0:j], x=t_shot[0:j]) / (rho * diameter ** 2)
        ...
        axes_h[1, i].text(
            ...
            ha='left', va='bottom', transform=axes_h[1, i].transAxes,
            fontsize=10,
        )
        ...
        axes_h[1, i].plot(t_shot, h*10, c='C1', label=f'Shot #{shot}')
        axes_h[1, i].fill_between(t_shot, (h-h_delta)*10, (h+h_delta)*10, color='C1', alpha=0.2)
        axes_h[1, i].set_xlabel('Time (s)')
        axes_h[1, i].set_ylabel('h(t) (mm)')
        axes_h[1, i].set_ylim(0, 2.2)
    ...
    path_to_output_folder = Path(current_folder) / 'heatload_model_v2'
    path_to_output_folder.mkdir(parents=True, exist_ok=True)
    path_to_csv = path_to_output_folder / 'mass_loss_heatload_model_v2.csv'
    # with open(path_to_csv, 'w', newline='') as csvfile:
    #     csvfile.write('# -'*40 + ' #\n')
    #     csvfile.write('# \n')
    #     csvfile.write(f'# C = {mu:.4f}\n')
    #     csvfile.write(f'# rho = {rho:.4f}\n')
    #     csvfile.write(f'# rho_delta = {rho_delta:.4f}\n')
    #     csvfile.write(f'# h0 = {h0:.4f}\n')
    #     csvfile.write(f'# h0_error = {h0_error:.4f}\n')
    #     csvfile.write(f'# theta = {np.degrees(theta):.4f}\n')
    #     csvfile.write('# -'*40 + ' #\n')
