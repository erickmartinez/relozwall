...
def main(
    ...
    fig_ncols = len(shots)
    fig, axes = plt.subplots(nrows=3, ncols=fig_ncols, constrained_layout=True, sharex=False)
    ...
    for i, shot in enumerate(shots):
        ...
        axes[0].plot(t_lp+t0, current_interp, c='C0', label=f'dro1')
        axes[1].plot(t_lp+t0, T_eV, c='C1', label=f'LP')
        axes[1].plot(t_lp+t0, T_eV_smooth, c='tab:red', label=f'Smoothened')
        axes[2].plot(t_lp+t0, I_x_TeV, c='C2', label=f'Product')
    ...
    axes[1].set_ylabel(r'{ T\textsubscript{e} (eV)}', usetex=True)
    axes[2].set_ylabel(r'{ I x T\textsubscript{e} (eV$\cdot$C/s)}', usetex=True)

    fig_h, axes_h = plt.subplots(nrows=3, ncols=fig_ncols, constrained_layout=True, sharex=True)
    fig_h.set_size_inches(4.5, 6.5)
    ...
    for i, shot in enumerate(shots):
        ...
        axes_h[0].text(
            0.025, 0.975, mass_loss_txt,
            ha='left', va='top', transform=axes_h[0].transAxes,
            fontsize=10,
        )
        ...
        height_loss_txt = f'Height loss: {height_loss*10:.2f} ± {height_loss_error*10:.2f} mm'

        axes_h[2].text(
            0.975, 0.975, height_loss_txt,
            ha='right', va='top', transform=axes_h[2].transAxes,
            fontsize=10,
        )


        axes_h[0].plot(t_shot, dm_dt, c='C0', label=f'Shot #{shot}')
        axes_h[0].fill_between(t_shot+t0, (dm_dt - dm_dt_error), (dm_dt + dm_dt_error), color='C0', alpha=0.2)

        # axes_h[0].set_xlabel('Time (s)')
        axes_h[0].set_ylabel('dm/dt (g/s)')
        axes_h[0].set_xlim(t_shot.min(), t_shot.max())

        # axes_h[0, i].legend(loc='upper right')
        axes_h[0].set_title(f'Shot #{shot}')
        axes_h[0].set_ylim(0, 0.1)

        axes_h[1].plot(t_shot, mass_loss_at_time, c='C3', label=f'Shot #{shot}')
        axes_h[1].fill_between(t_shot, (mass_loss_at_time - m_delta), (mass_loss_at_time + m_delta), color='C3', alpha=0.2)
        # axes_h[1].set_xlabel('Time (s)')
        axes_h[1].set_ylabel('{ $\Delta$m(t) (g)}', usetex=True)
        axes_h[1].set_ylim(bottom=0, top=0.04)

        axes_h[2].plot(t_shot, h*10, c='C4', label=f'Shot #{shot}')
        axes_h[2].fill_between(t_shot, (h-h_delta)*10, (h+h_delta)*10, color='C4', alpha=0.2)
        axes_h[2].set_xlabel('Time (s)')
        axes_h[2].set_ylabel('h(t) (mm)')
        axes_h[2].set_ylim(0, 2.2)
        # axes_h[1, i].set_title(f'Shot #{shot}')

    ax_g1 = axes_h[0].secondary_yaxis('right', functions=(
        grams_per_second_to_atoms_per_second, atoms_per_second_to_grams_per_second
    ))

    ax_g1.set_ylabel(r'{ x10\textsuperscript{22} B atoms/s}', usetex=True)

    # for i in range(3):


    shots_txt = f'{shots[0]}-{shots[-1]}' if len(shots) > 1 else fr'{shots[0]}'
