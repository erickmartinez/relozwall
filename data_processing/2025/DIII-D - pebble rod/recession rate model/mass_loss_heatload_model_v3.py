...
def main(
    ...
    fig_h, axes_h = plt.subplots(nrows=3, ncols=2, constrained_layout=True, sharex=True)
    fig_h.set_size_inches(7.5, 6.5)
    ...
    for i, shot in enumerate(shots):
        ...
        axes_h[1, i].plot(t_shot, mass_loss_at_time*1000, c='C3', label=f'Shot #{shot}')
        axes_h[1, i].fill_between(t_shot, (mass_loss_at_time - m_delta)*1000, (mass_loss_at_time + m_delta)*1000, color='C3', alpha=0.2)
        axes_h[1, i].set_xlabel('Time (s)')
        axes_h[1, i].set_ylabel('{\sffamily $\Delta$m(t) (mg)}', usetex=True)
        axes_h[1, i].set_ylim(bottom=0, top=30)
