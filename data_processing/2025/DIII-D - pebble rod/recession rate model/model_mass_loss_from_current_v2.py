...
CURRENT_DATA_DIR = r'./data/baselined_sweep'
...
class MassLossFromCurrent(object):
    ...
    def current_to_mass_loss(self, current):
        ...
        print(f'diameter: {diameter:.3f},-/+ {diameter_delta:.3f} cm')
...
def main(shot, model_params, current_data_dir, boron_molar_mass, figures_dir):
    ...
    path_to_integrated_current_csv = Path(model_params).parent.parent / f'{shot}_integrated_current.csv'
    t_range = load_trange(path_to_integrated_current_csv)


    msk_t_range = (t_range[0]*1E-3 <= t_s) & (t_s <= t_range[1]*1E-3)
    t_s, current = t_s[msk_t_range], current[msk_t_range]
    ...
    path_to_figures = Path(figures_dir) / 'mass_loss_model_v2'
