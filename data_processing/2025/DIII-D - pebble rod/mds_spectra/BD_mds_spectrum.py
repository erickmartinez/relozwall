...
def main(shot, wl_sum_range, path_to_data, line_label, diameter_mds_spot, time_max, make_movie=MAKE_MOVIE):
    ...
    # Load T_e from the LP data to estimate S/XB
    ...
    peak_distance = 50
    prominence = 8
    idx_peaks_qpara, _ = find_peaks(T_eV_smooth, prominence=prominence, distance=peak_distance)
    # reduce the search for times between 1 and 3 seconds
    idx_peaks_qpara = np.array(
        [id_peak for id_peak in idx_peaks_qpara if (t_peak_min <= t_lp[id_peak]) & (t_lp[id_peak] <= t_peak_max)])

    # Get qpara at these times
    qpara_osp_hits = qpara[idx_peaks_qpara]
    # Find the peaks for the ionized emission flux
    idx_peaks_bd, _ = find_peaks(flux_bd, distance=40, prominence=20)
    idx_peaks_bd = np.array(
        [id_peak for id_peak in idx_peaks_bd if (t_peak_min <= time_s[id_peak]) & (time_s[id_peak] <= t_peak_max)])
    flux_bd_osp_hits = flux_bd[idx_peaks_bd]

    if shot == 203784:
        t_peak_min, t_peak_max = 2, 3.15
        idx_peaks_qpara = (t_peak_min <= t_lp) & (t_lp <= t_peak_max)
        idx_peaks_bd = (t_peak_min <= time_s) & (time_s <= t_peak_max)
    ...
    if shot != 203784:
        ax3.plot(t_lp[idx_peaks_qpara], spl_TeV(t_lp)[idx_peaks_qpara], color='red', marker='x', ls='none', label='OSP at DiMES')
    if shot == 203784:
        ax3.axvspan(xmin=t_lp[idx_peaks_qpara][0], xmax=t_lp[idx_peaks_qpara][-1], color='tab:red', alpha=0.25, label='OSP at DiMES' )
    ...
    if shot != 203784:
        ax4.plot(time_s[idx_peaks_bd], flux_bd[idx_peaks_bd], label='OSP at DiMES', marker='x', ls='none')
    if shot == 203784:
        ax4.axvspan(xmin=time_s[idx_peaks_bd][0], xmax=time_s[idx_peaks_bd][-1], color='tab:red', alpha=0.25, label='OSP at DiMES' )
    ...
    with open(path_to_csv_out, 'w') as f:
        ...
        if shot != 203784:
            emission_flux_vs_qpara_df = pd.DataFrame(data={
                'time (s)': time_s[idx_peaks_bd],
                'qpara (MW/m2)': qpara_despiked[idx_peaks_qpara],
                'Flux (molecules/s)': flux_bd[idx_peaks_bd],
            })
        if shot == 203784:
            # Which dataset has the larger length
            qpara_size = len(qpara_despiked[idx_peaks_qpara])
            bd_size = len(flux_bd[idx_peaks_bd])
            if bd_size > qpara_size:
                # interporalate bd_size into len of qpara
                f = interp1d(y=flux_bd, x=time_s, fill_value=0)
                time_save = t_lp[idx_peaks_qpara]
                flux_bd_save = f(t_lp[idx_peaks_qpara])
                qpara_save = qpara_despiked[idx_peaks_qpara]
            elif bd_size < qpara_size:
                # interpolate qpara into bd_size
                f = interp1d(y=qpara_despiked, x=t_lp, fill_value=0)
                time_save = time_s[idx_peaks_bd]
                qpara_save = f(time_s[idx_peaks_bd])
                flux_bd_save = flux_bd[idx_peaks_bd]
            else:
                qpara_save = qpara_despiked[idx_peaks_qpara]
                flux_bd_save = flux_bd[idx_peaks_bd]
                time_save = time_s[idx_peaks_bd]
            emission_flux_vs_qpara_df = pd.DataFrame(data={
                'time (s)': time_save,
                'qpara (MW/m2)': qpara_save,
                'Flux (molecules/s)': flux_bd_save,
            })
