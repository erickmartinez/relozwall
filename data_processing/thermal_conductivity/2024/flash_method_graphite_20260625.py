...
def main():
    ...
    for i in range(poly_n):
        ...
        if i + 1 < poly_n:
            fit_txt += '\n'
    chi = density * cp * thickness_cm / popt[0]
    chi_err = chi * np.linalg.norm(
        [delta_popt[0] / popt[0], density_error / density, cp_err / cp, thickness_err_cm / thickness_cm])
    fit_txt += '\n'
    fit_txt += r"$q_{\mathrm{L}} = (LDC_{\mathrm{p}}/\chi) \Delta T_{\mathrm{max}}$" + '\n'
    fit_txt += fr'$\chi = {chi:.2f} \pm {chi_err:.2f}$'
