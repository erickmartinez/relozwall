...
def tlpmsqps2apmsqps(deposit_rate):
    return deposit_rate * 3.22E19

def apmsps2tlpmsqps(depisit_rate):
    return depisit_rate / 3.22E19
...
def main(base_dir, database_csv, laser_power_csv, csv_soot_deposition, drive_path, h0, h0_err, ncos, dncos):
    ...
    secax = ax.secondary_yaxis('right', functions=(tlpmsqps2apmsqps, apmsps2tlpmsqps))
    secax.set_ylabel(r'atoms/m$^{\mathregular{2}}$/s')
    ...
    print(f'sublimation_boron_rate {sublimation_boron_rate.min():.3f}')
    thickness_boron, thickness_boron_error = 0.0055, 0.0055
    adjustement_factor_carbon_to_boron =  12.011/10.811*1.6/2.2
    boron_empty = np.ones_like(hl_boron)
    deposition_boron_rate, deposition_boron_rate_err = thickness_boron/1.*boron_empty, thickness_boron_error/1.* boron_empty
    sublimation_boron_rate, sublimation_boron_rate_err = deposition_rate_to_sublimation_rate(
        deposition_rate=deposition_boron_rate,
        deposition_rate_error=deposition_boron_rate_err,
        h0=h0, dh0=h0_err, n=ncos, dn=dn_cos, area=pebble_area
    )
    sublimation_boron_rate *= adjustement_factor_carbon_to_boron
    sublimation_boron_rate_err *= adjustement_factor_carbon_to_boron
    print(f'sublimation_boron_rate {sublimation_boron_rate.min():.3f}')
    ...
    print(f'rs_boron {rs_boron.min():.3f}')
    rs_boron = sublimation_boron_rate
    rs_boron_error = sublimation_boron_rate_err
    print(f'rs_boron {rs_boron.min():.3f}')
    ...
    ax.set_xlim(5, 40)
    ax.tick_params(which='both', axis='y', labelright=False, direction='out')
