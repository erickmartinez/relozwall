...
def get_mean_power(laser_power_measurement: np.ndarray):
    max_power = laser_power_measurement.max()
    msk_on = laser_power_measurement > 0.7 * max_power
    laser_power_on = laser_power_measurement[msk_on]
    mean_power, std = laser_power_on.mean(), laser_power_measurement.std(ddof=1)
    n = len(laser_power_on)
    tval = t.ppf(1. - 0.5 * 0.05, n - 1)
    power_se = tval * std / np.sqrt(n)
    return mean_power, power_se
...
def main():
    ...
    pattern = re.compile(r'DiMES_HEAD_N(\d+)')
    ...
    for i, fn in enumerate(list_files):
        ...
        head_number = pattern.match(sample_name).group(1)
        msk_power = laser_power > 0.
        time_pulse = time_s[msk_power]
        mean_power, power_se = get_mean_power(laser_power)
        ...
        temperature_raw = np.array([round(x*4.)/4. for x in temperature_raw])
        ...
        axes[0].plot(
            time_s, temperature_raw, color=colors[i], label=fr'Head #{head_number} ({mean_power*1E-3:.1f} kW)'
        )
        ...
        axes[1].plot(
            time_s, temperature_raw-temperature_raw[0], color=colors[i], label=fr'Head #{head_number}'
        )
    ...
    print(output_df)
    print(delta_df)
    output_df.to_csv(os.path.join(output_data_dir, 'raw_temperatures_heads.csv'), index=False, encoding="utf-8-sig")
    columns = delta_df.columns
    descriptive_df = delta_df[columns[1::]].apply(pd.DataFrame.describe, axis=1)
    delta_df.to_csv(os.path.join(output_data_dir, 'del_temperature_heads.csv'), index=False, encoding="utf-8-sig")
    ...
    fig.savefig('./figures/dimes_temperatures_heads.png', dpi=600)
