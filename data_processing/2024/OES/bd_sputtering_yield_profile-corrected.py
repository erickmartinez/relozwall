...
SPUTTERING_RATES_CSV = r'./data/bd_sputtering_yields.csv'
...
def main(sputtering_yield_file, color_mapping, axes_mapping, label_mapping, marker_mapping):
    ...
    for i, folder in enumerate(folders):
        ...
        ax_y.plot(xpred / 60., np.exp(model_poly(xpred, fit_result_y.x)), color=colors[i], ls='--', lw=1)
        ax_g.set_yscale('log')
        # ax_g.legend(loc='upper right', frameon=True, fontsize=10)
        ax_g.set_ylim(1E10, 1E13)

        ax_y.set_yscale('log')
        # ax_y.legend(loc='lower left', frameon=True, fontsize=10)
        ax_y.set_ylim(1E-7, 1E-4)
    ...
    for ax in axes[:,0]:
        ax.set_ylabel(r"$\Gamma_{\mathrm{BD}}$ {\sffamily (cm\textsuperscript{-2} s\textsuperscript{-1})}", usetex=True)
        # ax.axhline(y=6E-3, ls='--', lw=1., color='0.5')
        handles, labels = ax.get_legend_handles_labels()
        ax.legend(handles[::-1], labels[::-1], loc='upper right')
    for ax in axes[:,1]:
        ax.set_ylabel(r'$Y_{\mathrm{BD/D^+}}$', usetex=True)
        # ax.axhline(y=6E-3, ls='--', lw=1., color='0.5')
