...
class ThermalDesorptionSimulator:
    ...
    def __init__(
            ...
            kr=3.2e-14,  # Recombination coefficient (m³/s)
            Er=1.,  # The recombination energy (eV)
    ...
    def estimate_CT0(self):
        ...
        if hasattr(self, 'gamma_ampl') and self.gamma_ampl > 0 and 1==1:
    ...
    def pde_system(self, tau, y):
        ...
        # dudt[0] = 2.0 * D_ratio * self.L * self.K_r(T) * u[0]**2  # Original incorrect units

        # Corrected left boundary with proper units
        # Following the derivation: J = -D ∂C/∂x = 2*Kr*C^2 at x=0
        # In normalized form: dudt[0] = 2 * Kr * C0 * u[0]^2 / D(T)
        # dudt[0] = 2.0 * self.K_r(T) * self.C0 * self.L * u[0]**2 / self.D(T)
        dx_first = self.X[1] - self.X[0]
        dudt[0] = D_ratio * (u[1] - 2 * u[0]) / dx_first ** 2 # Assume u[0-dx] = 0
        # dudt[0] =  -4. * D_ratio * self.L * self.C0 * self.K_r(T) * u[0] * (u[1] -  2 * u[0]) / dx_first ** 2 / self.D(T)
    ...
    def simulate(self, t_max=None):
        ...
        ntaus_max = 10000  # Upper limit to prevent excessive computation
        ...
        solution = solve_ivp(
            ...
            max_step=tau_max / 100,  # Maximum step size
