...
class TDSModel:
    def __init__(self,
                 ...
                 Etrap=0.3,  # Trapping energy (eV)
                 c0=1e24,  # Initial concentration (atoms/m³)
                 ntrap=1e22,  # Trap density (traps/m³)
    ...
    def simulate(self):
        ...
        dt = 0.01
        # self.t = np.linspace(0, tmax, 100000)
        NSTEPS = tmax // dt + 1
        self.t = np.arange(NSTEPS) * dt
        ...
        self.flux = np.array([
            -self.D(T) * (self.solution[i, 0] - self.solution[i, 2]) / (2 * self.dx)
    ...
    def plot_results(self):
        ...
        plt.plot(self.T, self.flux, 'b-', label='Desorption flux')
        ...
        # Also plot concentration profiles at different times
        plt.figure(figsize=(10, 6))
