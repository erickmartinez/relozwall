...
TDS_FILE = r'./data/Kikuchi2009/Kikuchi JNM 2009 - D2-TDS.csv'
RESULTS_FOLDER = r'./data/Kikuchi2009/fit_results'
...
def get_deuterium_retention(tds_df: pd.DataFrame, tds_ramp_rate):
    ...
    deuterium_flux = tds_df['Desorption rate (D2/m^2/s)'].values
...
def load_tds_data(path_to_csv) -> pd.DataFrame:
    df: pd.DataFrame = pd.read_csv(path_to_csv, comment = '#').apply(pd.to_numeric)
    df.sort_values(by=['Temperature (K)'], ascending=True, inplace=True)
    # df['Desorption rate (D2/m^2/s)'] = df['Desorption rate (D2/cm^2/s)'] * 1E4
    return df
...
def main(path_to_csv, path_to_results, tds_ramp_rate):
    hammada_df = load_tds_data(path_to_csv)

    """
    Fit the data for D+ implanted on hBN at 20 keV  with single desorption peak of order 1
    """
    x_data_boron = hammada_df['Temperature (K)'].values
    y_data_boron = hammada_df['Desorption rate (D2/m^2/s)'].values
    ...
    fit_result: OptimizeResult = fit_tds(
        xdata=x_data_boron, ydata=y_data_boron, x0=x0, bounds=bounds, loss='soft_l1', f_scale=0.1
    )

    x_pred = np.linspace(300, 1200, num=1000)
    ...
    save_fit_results(
        ...
        d_retention=get_deuterium_retention(tds_df=hammada_df, tds_ramp_rate=tds_ramp_rate)
    )

    y_pred, delta = cf.prediction_intervals(
        model=model_sum1, x_pred=x_pred, ls_res=fit_result, jac=jac_sum1
    )
    ...
    ax.plot(
        hammada_df['Temperature (K)'].values,
        hammada_df['Desorption rate (D2/m^2/s)'].values, marker='o',
        color='C0', mew=1., mfc='none', ls='none', ms=7,
        label='Total D'
    )

    ax.plot(x_pred, y_pred, color='tab:red')
    plot_fit_model(ax=ax, fit_result=fit_result, x_pred=x_pred)


    ax.set_title(r'{\sffamily D\textsuperscript{+} → B Kikuchi 1 keV (2009)}', usetex=True)
