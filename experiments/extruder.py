...
class ExtrusionProcedure(Procedure):
    ...
    dc_source_mode = ListParameter('Voltage Mode', choices=['PID', 'Constant Voltage'], default='PID')
    constant_dc_voltage = FloatParameter('DC Voltage', minimum=0.0, maximum=150.0, default=0.0)
    temperature_setpoint = FloatParameter('Temperature setpoint', units='C', default=20.0, minimum=0.0, maximum=900.0)
    ...
    DATA_COLUMNS = ["Time (s)", "Baking Pressure (Torr)", "Outgassing Pressure (Torr)", "Baking Temperature (C)",
                    "Outgassing Temperature (C)", "Position (in)", "DC Voltage (V)", "Is Baking?"]
    ...
    def execute(self):
        ...
        if self.dc_source_mode == 'PID':
            ...
            while run_pid:
                ...
                if (not ramping) and (not stabilizing) and (not baking) and (not moving) and delta_x_cm > 0.1:
                    self.__translator.move_by_time(moving_time=moving_time, speed_setting=self.__velocity_adc_setpoint)
                    moving = True
                    moving_t0 = current_time

                if (not ramping) and (not stabilizing) and (not baking) and moving and current_position + self.sample_length > self.baking_position and not baking:
                    baking = True
                ...
                if (current_time - previous_time) >= dt:
                    ...
                    d = {
                        ...
                        'Position (in)': current_position/2.54,
            ...
            total_time = 0.0
            start_time = time.time()
            self.__dc_source.voltage_setpoint = self.constant_dc_voltage
            while total_time <= self.bake_time*60:
                if self.should_stop():
                    log.warning("Caught the stop flag in the procedure")
                    break
                current_time = time.time()
                ...
                if (current_time - previous_time) >= dt:
                    ...
                    d = {
                        ...
                        "Baking Temperature (C)": TC1,
                        "Outgassing Temperature (C)": TC2,
                        "DC Voltage (V)": self.constant_dc_voltage,
                        "Is Baking?": 1.0
                    }
                    self.emit('results', d)
                    self.emit('progress', 100.0 * total_time / experiment_time)
                    print(f"T = {TC1:>6.2f} °C, Baking Time: {total_time:>5.2f} s", end='\r',
                          flush=True)
                    previous_time = current_time
            log.info("Turning off the ouput of the DC power supply.")
            self.__dc_source.voltage_setpoint = 0.0
...
class MainWindow(ManagedWindow):
    ...
    def __init__(self):
        super(MainWindow, self).__init__(
            ...
            inputs=['sample_name', 'bake_time', 'dc_source_mode', 'constant_dc_voltage', 'temperature_setpoint', 'temperature_ramp_rate', 'temperature_stabilization_time', 'baking_position', 'sample_length'],
            displays=['sample_name', 'bake_time', 'dc_source_mode', 'constant_dc_voltage', 'temperature_setpoint', 'temperature_ramp_rate', 'temperature_stabilization_time', 'baking_position', 'sample_length'],
            x_axis="Time (s)",
    ...
    def queue(self):
        ...
        voltage_setpoint = procedure.constant_dc_voltage

        if procedure.dc_source_mode == 'PID':
            prefix = f'EXTRUSION_{sample_name}_{temperature_setpoint:04.0f}C'
        else:
